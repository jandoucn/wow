...
wowam.ClassBeeScript.FUNCTIONS=function()
...
function Beemapconfig(self,...)
	local SetCVar=SetCVar;
	local debugprofilestop=debugprofilestop;
	RegisterCVar("OnUpdateTime", 0);SetCVar("OnUpdateTime", 0);
	RegisterCVar("PLAYER_LOGOUT", 0);SetCVar("PLAYER_LOGOUT",0);
	RegisterCVar("PLAYER_ENTERING_WORLD", 0);SetCVar("PLAYER_ENTERING_WORLD",0);
	RegisterCVar("PlayerGuid", "");SetCVar("PlayerGuid", "");
	RegisterCVar("IsBNLogin", "");SetCVar("IsBNLogin", tostring(IsBNLogin));
	RegisterCVar("ADDON_LOADED", 0);SetCVar("ADDON_LOADED", debugprofilestop());
	_amVar={};
	local Frame=CreateFrame('Frame');
	local tostring=tostring;
	
	Frame:SetScript('OnUpdate',function() 
		SetCVar("OnUpdateTime", debugprofilestop());	
		--if not __hver and GetFireHackVersion then 
		--	fhver=GetFireHackVersion();
		--end
		if not __hver then
		 islong=IsBNLogin();
		end
		
		for name, v in pairs(_amVar) do
			SetCVar(name, tostring(_G[name] or "err"));			
		end	
	...
	Frame:SetScript('OnEvent',function(self, event, ...)
		...
		if event=="PLAYER_LOGOUT" then
			SetCVar("PLAYER_LOGOUT",debugprofilestop());
			SetCVar("PLAYER_ENTERING_WORLD",0);
			SetCVar("PlayerGuid", "");
		elseif event=="PLAYER_ENTERING_WORLD" then
			SetCVar("PLAYER_ENTERING_WORLD",debugprofilestop());
			SetCVar("PLAYER_LOGOUT",0);
			SetCVar("PlayerGuid", UnitGUID("Player"));
		elseif event=="ADDON_LOADED" then
			SetCVar("ADDON_LOADED",debugprofilestop());
		end
	end)
end;Beemapconfig();
...
local spll=1;
CreateFrame('Button', '__TusSister', UIParent, 'SecureActionButtonTemplate');
__TusSister:SetAttribute('type', 'macro');
__TusSister:SetAttribute('macrotext', '/run tostring(' ..spll..')' ..string.rep(' ', 1024));
SetOverrideBindingClick(__TusSister, true, 'Insert', __TusSister:GetName());
RegisterCVar('GetAttribute', '');SetCVar('GetAttribute', tostring(__TusSister.GetAttribute));
...
function BeeChannelSend(channel,String,Time)
	...
	beeps_T,beeps_F=beeps_T or 0,beeps_F or CreateFrame("frame")
	...
	if beeps_X then 
		...
		beeps_X=function()
			local t=GetTime()
			if t- beeps_T > beeps_Time then 
				SendChatMessage(beeps_s,"channel",nil,beeps_SCID)
				beeps_T=t 
			end 
		end 
	end beeps_F:SetScript("OnUpdate",beeps_X)
